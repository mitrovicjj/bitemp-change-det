...
from src.oscd_msi import OSCDDataset
...
class UNet(nn.Module):
    def __init__(self, in_channels=26, out_channels=1, features=(64, 128, 256, 512)):
        super().__init__()
...
def main(args):
    ...
    train_ds = OSCDDataset(
        ...
        selected_bands=args.selected_bands,
    )
    val_ds = OSCDDataset(
        split="test",
        patch_size=args.patch_size,
    ...
    model = UNet(in_channels=train_ds.input_channels, out_channels=1, features=(32, 64, 128, 256)).to(device)
    ...
    best_path = out_dir / "best_unet_oscd_msi.pt"
    last_path = out_dir / "last_unet_oscd_msi.pt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train U-Net on OSCD MSI with MLflow logging (CPU only)"
    )
    ...
    parser.add_argument("--run-name", type=str, default="msi_unet_cpu")
    parser.add_argument("--model-name", type=str, default="unet")
    parser.add_argument("--dataset-name", type=str, default="blanchon/OSCD_MSI")
    parser.add_argument("--mlflow-tracking-uri", type=str, default="file:./mlruns")
    parser.add_argument("--normalize", type=str, default="scale_10000")
    parser.add_argument(
        "--selected-bands",
        type=int,
        nargs="*",
        default=None,
        help="Optional band indices to use, e.g. --selected-bands 1 2 3",
    )
